...
def get_profile(enrollment_number):
    try:
        file_content = repo.get_contents(PROFILE_FILE).decoded_content.decode('utf-8')
        profiles = json.loads(file_content)
        profile = profiles.get(enrollment_number)
        return profile
    ...
    if uploaded_file:
        ...
        if timetable_sheet is not None and subjects_sheet is not None:
            ...
            if enrollment_number:
                profile = get_profile(enrollment_number)

                if profile:
                    selected_section = profile.get("section")
                    selected_subjects = profile.get("selected_subjects")

                    if selected_section and selected_subjects:
                        ...
                        st.error("Profile data is incomplete.")
                else:
                    st.warning("Profile not found. Please ensure the enrollment number is correct.")
        else:
